...
import { trackRecentAgent } from '@/lib/recent';
...
export default function ChatPage() {
  ...
  useEffect(() => {
    if (initialPrompt && agent && messages.length <= 1) {
  ...
  const categoryColor = agent ? CATEGORY_COLORS[agent.category || ''] || '#6366f1' : '#6366f1';
  const suggestedPrompts = useMemo(() => {
    if (!agent) return ['你能帮我做什么？', '给我介绍一下你的能力', '我们从一个小任务开始'];
    return promptMap[agent.category || ''] || ['你能帮我做什么？', '给我介绍一下你的能力', '我们从一个小任务开始'];
  }, [agent]);
  ...
  const handleSend = async (text?: string) => {
    ...
    try {
      ...
      setStreamingContent('');
      trackRecentAgent(agentId, content);
    } catch (error: any) {
      ...
      <aside className="hidden w-[340px] shrink-0 border-r border-black/[0.06] bg-white/80 p-5 backdrop-blur lg:flex lg:flex-col">
        ...
        <div className="overflow-hidden rounded-[32px] border border-black/[0.06] bg-[#fbfaf7] shadow-sm">
          ...
          <div className="p-5 text-center">
            ...
            <h1 className="mt-3 line-clamp-2 text-xl font-black leading-tight text-slate-950">{agent.name}</h1>
      ...
      <main className="flex min-w-0 flex-1 flex-col">
        <header className="flex items-center gap-3 border-b border-black/[0.06] bg-white/86 px-4 py-3 backdrop-blur lg:hidden">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="truncate text-sm font-black text-slate-950">{agent.name}</h1>
            <p className="text-xs font-medium text-slate-400">{agent.category} · {agent.tone}</p>
          </div>
        </header>
            ...
            {messages.map((message) => (
              <div key={message.id} className={cn('flex gap-3', message.role === 'user' ? 'justify-end' : 'justify-start')}>
                {message.role === 'assistant' && (
                  <Avatar src={agent.avatar} alt={agent.name} size="sm" className="mt-1 shrink-0" />
                )}
                <div
                  className={cn(
