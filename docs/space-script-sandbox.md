...
- 使用 `--unshare-all` 建立隔离命名空间；允许联网的空间自动化随后通过 `--share-net` 保留宿主网络，禁止联网的自动化和 Space Skill 维持独立网络命名空间。
...
| 联网策略 | Bubblewrap 配置 | 脚本行为 |
| --- | --- | --- |
| 禁止联网 (`forbidden`) | `--unshare-net` | 不能访问公网、宿主回环地址或局域网 |
| 允许按需联网 (`allowed`) | `--share-net` | 脚本可以按需访问网络 |
| 必须联网 (`required`) | `--share-net` | 为脚本保留网络；运行时不判断脚本是否实际发起请求 |

用户可以在自动化编辑界面中选择策略。新建自动化默认禁止联网，需要采集网页或调用第三方 API 时再显式开启。启用后，脚本也可以访问宿主回环地址和服务器可达的内网服务。

文件系统、环境变量、Linux capabilities 和进程命名空间隔离仍然生效，但 Bubblewrap 本身不提供域名或目标 IP 级别的网络过滤。脚本联网意味着仍存在 SSRF、内网探测和数据外传风险；不要向脚本注入不必要的凭据。后续可以增加自动化级的联网授权和受控出口代理，进一步限制目标地址。

Space Skill Runner 继续使用 `--unshare-net` 强制断网。Skill 脚本不能访问公网、宿主回环地址或局域网服务。
...
先确认自动化联网策略不是“禁止联网”，再检查服务器 DNS、防火墙、出口代理和目标服务策略。Space Skill 固定断网，不受自动化联网策略影响。
