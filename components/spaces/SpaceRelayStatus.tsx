...
export default function SpaceRelayStatus({ relay, agents, busy, onAction, onOpen, onDismiss }: {
  ...
  agents: Agent[];
  busy: boolean;
  onAction: (action: 'cancel' | 'pause' | 'resume' | 'approve' | 'reject' | 'continue' | 'stop') => void;
  onOpen: () => void;
  onDismiss: () => void;
}) {
  ...
  const stageLimitPending = relay.pendingAction?.type === 'stage_limit';
  const stage = relayStagePolicy(relay.kind);
  const board = relay.kind === 'gomoku' && state?.board && Array.isArray(state.board) ? state.board as number[] : null;
  ...
  const title = relay.status === 'COMPLETED' ? '接力已完成'
    : relay.status === 'CANCELLED' ? '接力已停止'
      : relay.status === 'FAILED' ? '接力失败'
        : relay.status === 'PAUSED' ? '接力已暂停'
          : relay.status === 'PAUSE_REQUESTED' ? '本轮结束后暂停'
        : pendingContinuation ? '协调者建议继续一轮'
          : summarizing ? '空间协调者正在验收'
            : relay.status === 'WAITING_APPROVAL' ? (stageLimitPending ? '等待决定 · 阶段上限' : `等待确认 · ${relay.pendingAction?.agentName || currentAgent?.name || '成员'}`)
    ...
    <div className="border-y border-black/[0.06] bg-white px-3 py-3 sm:px-4">
      <div className="flex items-start gap-3">
        <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-slate-600">
          {active && !['WAITING_APPROVAL', 'PAUSED'].includes(relay.status) ? <Loader2 className="animate-spin" size={15} /> : <Repeat2 size={15} />}
        </div>
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="text-sm font-black text-slate-900">{title}</div>
            {active && relay.status !== 'WAITING_APPROVAL' && relay.status !== 'PAUSED' && (
              <div className="flex items-center gap-1">
                {['QUEUED', 'RUNNING'].includes(relay.status) && <button type="button" onClick={() => onAction('pause')} disabled={busy} className="inline-flex h-8 items-center gap-1.5 rounded-lg px-2 text-xs font-black text-slate-600 hover:bg-slate-100 disabled:text-slate-300">暂停</button>}
            ...
            {relay.status === 'PAUSED' && <button type="button" onClick={() => onAction('resume')} disabled={busy} className="h-8 rounded-lg bg-slate-950 px-3 text-xs font-black text-white disabled:bg-slate-200">继续</button>}
          ...
          {relay.status === 'WAITING_APPROVAL' && stageLimitPending && (
            <div className="mt-3 border-t border-black/[0.06] pt-3">
              <div className="text-xs font-semibold leading-5 text-slate-600">{relay.pendingAction?.summary}</div>
              <div className="mt-3 flex flex-wrap gap-2">
                <button type="button" onClick={() => onAction('continue')} disabled={busy} className="h-8 rounded-lg bg-slate-950 px-3 text-xs font-black text-white disabled:bg-slate-200">继续 {stage?.size} {stage?.unit}</button>
                <button type="button" onClick={() => onAction('stop')} disabled={busy} className="h-8 rounded-lg border border-black/[0.08] px-3 text-xs font-black text-slate-600 disabled:text-slate-300">结束接力</button>
              </div>
            </div>
          )}
          {relay.status === 'WAITING_APPROVAL' && relay.pendingAction && !stageLimitPending && (
            <div className="mt-3 border-t border-black/[0.06] pt-3">
              {pendingContinuation && (
                <>
                  <div className="text-xs font-semibold leading-5 text-slate-600">{relay.pendingAction.summary}</div>
