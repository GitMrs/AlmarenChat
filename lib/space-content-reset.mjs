...
async function stageEntries(root, stagedRoot, renamePath, preserveNames = new Set()) {
...
export async function resetSpaceContentsStorage(root, clearRecords, { renamePath = rename, preserveEntries = [] } = {}) {
  ...
  const preserveNames = new Set(preserveEntries);
  ...
  try {
    ...
    if (error?.code === 'ENOENT') {
      ...
      movedEntries = await stageEntries(resolvedRoot, stagedRoot, renamePath, preserveNames);
      stagingMode = 'entries';
    } else {
      throw error;
    }
