...
async function pruneDiskCache() {
  ...
  await Promise.all(files.slice(MAX_CACHE_ENTRIES).map(({ filePath }) => unlink(filePath).catch(() => {})));
}
...
export async function synthesizeSpeech(text, options = {}) {
  ...
  if (isCacheable && isDiskCacheEnabled(options)) {
  ...
  if (isCacheable && audioBuffer.length > 0) {
    ...
    if (isDiskCacheEnabled(options)) await persistDiskCache(cacheKey, audioBuffer);
  }
