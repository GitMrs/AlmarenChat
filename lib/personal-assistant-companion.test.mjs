...
import { buildTimedAssistantHistory } from './personal-assistant/history-context.mjs';

const projectRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');

test('assistant history preserves time evidence for stale transient states', async () => {
  const history = buildTimedAssistantHistory([
    { role: 'user', content: '等车中...', createdAt: new Date('2026-09-09T11:25:53.000Z') },
    { role: 'assistant', content: '我陪你等车。', createdAt: new Date('2026-09-09T11:26:00.000Z') },
  ]);

  assert.equal(history[0].content, '[历史消息时间：2026-09-09 19:25，北京时间]\n等车中...');
  assert.equal(history[1].content, '[历史消息时间：2026-09-09 19:26，北京时间]\n我陪你等车。');

  const [memoryRuntime, promptBuilder, webRoute, qqRoute] = await Promise.all([
  ...
  assert.match(memoryRuntime, /select: \{ role: true, content: true, createdAt: true \}/);
  assert.match(memoryRuntime, /具有时效性的状态表述/);
  assert.match(promptBuilder, /不能因为历史里没有明确的结束语/);
  assert.match(promptBuilder, /简短关联过往经历/);
  assert.match(promptBuilder, /不得在缺少新证据时将历史中的即时状态延续到当前/);
  assert.match(promptBuilder, /只描述该摘要覆盖时间结束时的状态/);
  assert.doesNotMatch(`${memoryRuntime}\n${promptBuilder}`, /等车|小面/);
  assert.match(webRoute, /buildTimedAssistantHistory\(memoryContext\.history\)/);
  assert.match(qqRoute, /buildTimedAssistantHistory\(memoryContext\.history\)/);
});
...
test('personal assistant web chat forwards uploaded images to both model modes', async () => {
  const [provider, api, route] = await Promise.all([
    readFile(path.join(projectRoot, 'components/assistant/PersonalAssistantProvider.tsx'), 'utf8'),
    readFile(path.join(projectRoot, 'lib/api.ts'), 'utf8'),
    readFile(path.join(projectRoot, 'app/api/assistant/messages/route.ts'), 'utf8'),
  ]);

  assert.match(provider, /uploads\.image\(file\)/);
  assert.match(provider, /onPaste=\{handleImagePaste\}/);
  assert.match(provider, /attachments: attachment \? \[attachment\] : undefined/);
  assert.match(provider, /attachments=\{message\.attachments\}/);
  assert.match(api, /attachments\?: MessageAttachment\[\]/);
  assert.match(route, /请分析这张图片。/);
  assert.match(route, /image_url/);
  assert.match(route, /attachments: imageAttachments\.length \? imageAttachments : undefined/);
});

test('global browser model settings live in personal center while agent settings stay read only', async () => {
  const [settings, agentPanel, chatRoom, provider, composer] = await Promise.all([
    ...
    readFile(path.join(projectRoot, 'components/assistant/PersonalAssistantProvider.tsx'), 'utf8'),
    readFile(path.join(projectRoot, 'components/chat/ChatComposer.tsx'), 'utf8'),
  ]);
  ...
  assert.match(agentPanel, /默认值来自个人中心，可在输入框切换/);
  ...
  assert.match(chatRoom, /modelSource === 'OLLAMA'/);
  assert.match(chatRoom, /onModelSourceChange/);
  assert.match(provider, /modelSource === 'OLLAMA'/);
  assert.match(provider, /setModelSource\('ONLINE'\)/);
  assert.match(provider, /setModelSource\('OLLAMA'\)/);
  assert.match(composer, /onModelSourceChange/);
  assert.match(composer, /线上/);
  assert.match(composer, /本地/);
});
...
test('personal assistant supports context transparency and manual experience archiving', async () => {
  const [memoryRuntime, experiencesRoute, experienceDetailRoute, api, provider, modal] = await Promise.all([
    readFile(path.join(projectRoot, 'lib/personal-assistant/experience-memory.ts'), 'utf8'),
    readFile(path.join(projectRoot, 'app/api/assistant/experiences/route.ts'), 'utf8'),
    readFile(path.join(projectRoot, 'app/api/assistant/experiences/[experienceId]/route.ts'), 'utf8'),
    readFile(path.join(projectRoot, 'lib/api.ts'), 'utf8'),
    readFile(path.join(projectRoot, 'components/assistant/PersonalAssistantProvider.tsx'), 'utf8'),
    readFile(path.join(projectRoot, 'components/assistant/AssistantContextModal.tsx'), 'utf8'),
  ]);

  assert.match(memoryRuntime, /export async function manualArchiveAssistantMessages/);
  assert.match(memoryRuntime, /unarchivedMessages\.slice\(0, unarchivedMessages\.length - preserveRecent\)/);
  assert.match(experiencesRoute, /export async function GET\(request: Request\)/);
  assert.match(experiencesRoute, /export async function POST\(request: Request\)/);
  assert.match(experiencesRoute, /estimateMessagesTokens/);
  assert.match(experienceDetailRoute, /export async function DELETE/);
  assert.match(api, /getContextStats:/);
  assert.match(api, /archiveExperience:/);
  assert.match(api, /deleteExperience:/);
  assert.match(provider, /AssistantContextModal/);
  assert.match(provider, /条活跃 · ~/);
  assert.match(provider, /以上早期对话已沉淀为经历记忆/);
  assert.match(modal, /沉淀为经历记忆（深度归档）/);
  assert.match(modal, /保留最近对话/);
  assert.match(modal, /已沉淀的经历记忆/);
});
