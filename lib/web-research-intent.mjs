...
const CONDITIONAL_SOURCE_PATTERN = /(?:若|如果|如有|如需|仅当).{0,32}(?:引用|来源|外部事实|外部资料)/i;
...
const RESEARCH_EXECUTION_DENIAL_PATTERN = /(?:无(?:需|须|任何)?|无需|不需要|不用|不要|不得|禁止|不允许|不必|避免|不(?:进行|使用|启用|访问|涉及)?)(?:(?!但|但是|不过|然而|仍需|还需|同时需要).){0,18}(?:联网|网上搜索|网络搜索|搜索|检索|调研|research)/i;
const OFFLINE_ARTIFACT_PATTERN = /(?:页面|网页|文件|产物|html).{0,20}(?:无需联网|不依赖网络|离线).{0,20}(?:打开|运行|预览)|(?:无需联网|不依赖网络|离线).{0,20}(?:打开|运行|预览).{0,20}(?:页面|网页|文件|产物|html)/i;
...
export function explicitlyForbidsResearchExecution(value) {
  const clauses = researchClauses(value);
  const hasPositiveResearch = clauses.some((clause) =>
    !RESEARCH_EXECUTION_DENIAL_PATTERN.test(clause) && EXPLICIT_WEB_RESEARCH_PATTERN.test(clause)
  );
  if (hasPositiveResearch) return false;
  return clauses.some((clause) =>
    RESEARCH_EXECUTION_DENIAL_PATTERN.test(clause) && !OFFLINE_ARTIFACT_PATTERN.test(clause)
  );
}

export function needsWebResearch(value) {
  const text = String(value || '');
  if (CONDITIONAL_SOURCE_PATTERN.test(text)
    && !EXPLICIT_RESEARCH_OVERRIDE_PATTERN.test(text)
    && !EXPLICIT_WEB_RESEARCH_PATTERN.test(text)) return false;
  const clauses = researchClauses(value);
  ...
  return clauses.some((clause) => {
    ...
    // Conditional output requirements do not request external research.
    if (CONDITIONAL_SOURCE_PATTERN.test(clause) && !EXPLICIT_RESEARCH_OVERRIDE_PATTERN.test(clause)) return false;
