...
async function copySharedWorkspace(resolved) {
  if (!resolved.worksRoot) return;
  const source = path.join(resolved.workspaceBase, 'shared');
  try {
  ...
  await copyWorkspace(source, path.join(resolved.stagingRoot, 'shared'));
}
...
export async function prepareWorkspaceAttempt(options, { sourceAttempt = null } = {}) {
  ...
  if (sourceAttempt === null) await copySharedWorkspace(resolved);
...
export async function saveExecutionCheckpoint(options, checkpoint) {
  ...
  try {
    ...
    if (!ready.isFile()) return { bytes: 0, skipped: true };
  } catch (error) {
    if (error?.code === 'ENOENT') return { bytes: 0, skipped: true };
    throw error;
  }
...
async function rollbackAppliedEntries(resolved, baselineByPath, entries) {
  for (const entry of [...entries].reverse()) {
    ...
    const target = workspaceTarget(resolved, relativePath);
...
function workspaceTarget(resolved, relativePath) {
  const logical = safeRelativePath(relativePath);
  return path.join(logical.startsWith('shared/') ? resolved.workspaceBase : resolved.workspaceRoot, logical);
}
...
export async function applyWorkspaceAttempt(options, baseline, entries) {
  ...
  for (const entry of changes) {
    const relativePath = safeRelativePath(entry.path);
    const targetRoot = relativePath.startsWith('shared/') ? resolved.workspaceBase : resolved.workspaceRoot;
    await assertNoSymlinkPath(targetRoot, relativePath);
    await assertNoSymlinkPath(resolved.stagingRoot, relativePath);
    const current = await fileSnapshot(workspaceTarget(resolved, relativePath));
  ...
  for (const entry of changes) {
    ...
    await copyFile(workspaceTarget(resolved, relativePath), backup);
  }
