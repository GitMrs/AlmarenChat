...
function directories(options) {
  ...
  const workspaceRoot = path.join(spaceRoot, 'workspace');
  ...
  return { spacesRoot, spaceRoot, workspaceRoot, stagingParent, taskRoot, attemptRoot, stagingRoot };
}
  ...
  const visit = async (sourceDirectory, targetDirectory) => {
    ...
    for (const entry of entries) {
      ...
      if (info.isDirectory()) {
        ...
        if (info.size > MAX_FILE_BYTES) throw new Error('工作区存在超过 512KB 的文件，无法创建安全暂存区');
...
export async function prepareWorkspaceAttempt(options) {
  ...
  await copyWorkspace(resolved.workspaceRoot, resolved.stagingRoot);
...
async function fileSnapshot(target) {
  try {
    ...
    const sha256 = info.size <= MAX_FILE_BYTES
