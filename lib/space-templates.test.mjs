...
test('built-in space templates have stable unique ids and bounded teams', () => {
  assert.equal(SPACE_TEMPLATES.length, 10);
...
test('all 10 built-in templates have specialized industrial-grade instructions', () => {
  for (const item of SPACE_TEMPLATES) {
    const instructions = spaceTemplateInstructions(item);
  ...
  // Gaming room
  const gaming = getSpaceTemplate('gaming-room');
  assert.match(gaming.deliverables.join('\n'), /match-report\.md/);
  assert.match(gaming.deliverables.join('\n'), /tactics-sheet\.md/);
  assert.match(spaceTemplateInstructions(gaming), /游戏开黑作战室运作指南/);
  assert.match(spaceTemplateInstructions(gaming), /战术复盘军师/);
});
...
test('gaming room template supports game companion banter and tactical review', () => {
  const item = getSpaceTemplate('gaming-room');
  assert.equal(item.version, 2);
  assert.equal(item.name, '游戏开黑作战室');
  assert.deepEqual(item.recommendedAgentIds, ['gaming-nox', 'gaming-koko', 'gaming-lulu']);
  assert.match(item.deliverables.join('\n'), /match-report\.md/);
  assert.match(item.deliverables.join('\n'), /tactics-sheet\.md/);
  const instructions = spaceTemplateInstructions(item);
  assert.match(instructions, /全员鲜明性格声线/);
  assert.match(instructions, /语音友好与对局快节奏规范/);
  assert.deepEqual(spaceTemplateSnapshot(item).qualityRules, item.qualityRules);
});
