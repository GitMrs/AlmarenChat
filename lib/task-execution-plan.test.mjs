...
test('keeps a validated advisor to executor chain', () => {
  ...
  assert.equal(defaultModelRequestLimit(tasks), 12);
});
...
test('rejects forward dependencies and write plans without an executor', () => {
  assert.throws(() => normalizeExecutionPlan({
    executionPlan: [{ agentId: 'product', mode: 'advisor', title: '规则', instruction: '规则', dependsOn: [0] }],
  }, members), /无效依赖/);
  assert.throws(() => normalizeExecutionPlan({
    capabilities: ['workspace_write'],
    executionPlan: [{ agentId: 'product', mode: 'advisor', title: '规则', instruction: '规则', dependsOn: [] }],
  }, members), /执行角色/);
});
...
test('assigns bounded run budgets by execution-chain complexity', () => {
  assert.equal(defaultModelRequestLimit([{ mode: 'executor' }]), 8);
  assert.equal(defaultModelRequestLimit([{ mode: 'executor' }, { mode: 'executor' }]), 16);
  assert.equal(defaultModelRequestLimit(Array.from({ length: 5 }, () => ({ mode: 'executor' }))), 24);
});
