...
export async function runPiSpaceTurn(options) {
  return exclusive(options.spaceId, async () => {
    ...
    const modelLimits = modelTokenLimits(options.modelName, options.modelContextWindow);
    ...
    const settingsManager = SettingsManager.inMemory({
      compaction: {
        enabled: true,
        reserveTokens: modelLimits.contextWindow - modelLimits.compactionTriggerTokens,
        keepRecentTokens: modelLimits.keepRecentTokens,
      },
            ...
            messages: compactSuccessfulPiMutationArguments(
              buildPiWorkingContext(
                isolatePiMultiReplyContext(
                  collapseCompletedPiScopes(event.messages, {
                    activeScopeId: options.coordinationScope?.scopeId,
                  }),
