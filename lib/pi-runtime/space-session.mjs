...
// Discussion has a separate user-started workflow. Pi may coordinate other
// member responses, but it must not create a discussion from ordinary chat.
...
export async function generatePiWorkspaceImage({ model, prompt, fileName, size, workspaceOptions, signal, fetchImpl = fetch }) {
  ...
  if (normalizedFileName === 'cover') {
    await Promise.all(['.png', '.jpg', '.jpeg', '.webp']
      ...
      .map((extension) => rm(path.join(piSpacePaths(workspaceOptions).workspaceRoot, 'assets', `cover${extension}`), { force: true })));
  }
...
export function createWorkspaceTools(options) {
  ...
  return [
    ...(options.roleAgent?.id === 'space-coordinator' && options.availableAgents?.length > 0 && typeof options.onCoordinationRequest === 'function' ? [defineTool({
      ...
      description: '仅当用户明确要求大家分别回应、协作评审、集体决策或接力创作时，邀请空间成员真实参与。讨论必须由用户从空间入口手动创建；普通问答、写文件和项目执行不得调用。',
      promptSnippet: 'Invite real space members to respond, review, decide, or relay; user-started discussions are separate',
      parameters: Type.Object({
        mode: Type.Union([
          Type.Literal('broadcast'), Type.Literal('review'), Type.Literal('decision'),
          Type.Literal('relay'),
        ]),
    ...
    ...(options.allowWebSearch && typeof options.webSearch === 'function' ? [defineTool({
      ...
      name: 'web_fetch', label: '读取网页', description: '读取指定公开 HTTPS 网页或 JSON 接口；禁止本机、内网和工作区地址。',
      promptSnippet: 'Fetch a specific public HTTPS URL when the user provided a link',
      parameters: Type.Object({ url: Type.String({ minLength: 1, maxLength: 2000 }) }, { additionalProperties: false }),
      execute: async (_toolCallId, params, signal) => {
        if (signal?.aborted) return toolResult({ error: '网页读取已取消' }, true);
        if (webSearchQueries.size >= 2) return toolResult({ error: '本轮最多允许两次联网操作，请使用已有资料完成任务' }, true);
        const url = String(params.url || '').trim();
        webSearchQueries.add(`fetch:${url.toLowerCase()}`);
        try {
          return toolResult(await fetchWebPage(url, { signal }));
        } catch (error) {
          return toolResult({ error: error instanceof Error ? error.message : String(error) }, true);
        }
      },
...
function roleSystemPrompt(options) {
  ...
  const coordinatorPrompt = roleAgent.id === 'space-coordinator'
    ? [
        ...
        interactionMode === 'chat' && availableAgents.length > 0
          ? '当用户明确要求成员参与，或使用“让大家……”“一起评审……”“投票选择……”“轮流续写……”等协作表达时，才调用 coordinate_members，让平台邀请真实成员；不得替成员编造回答。用户若要求“讨论”，不要调用 coordinate_members 的 discussion 模式，也不要自行开启讨论，必须提示用户从空间输入框的“发起讨论”入口手动创建。用户说“大家/所有成员”时必须传入全部可邀请成员 ID，否则按主题选择相关成员。普通问答和项目执行不要调用。'
          : '',
      ].filter(Boolean).join('\n')
  ...
  return [
    ...
    allowWebSearch
      ? '本轮已获得联网授权，可以使用 web_search 搜索或 web_fetch 读取用户给出的公开 HTTPS 网页；不得访问工作区、本机或内网。仍未开放任意终端、安装依赖或启动进程。'
