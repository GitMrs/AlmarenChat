...
test('negative workspace requirements remain read only', () => {
  ...
  assert.equal(needsWorkspaceWrite('不要修改任何文件；只读取 index.html 并回答'), false);
  assert.equal(needsWorkspaceWrite('无需写入工作区；检查 docs/report.md'), false);
  assert.equal(needsWorkspaceWrite('只读查看 index.html'), false);
});
...
test('positive workspace requirements still permit writes', () => {
  ...
  assert.equal(needsWorkspaceWrite('编写 app/page.tsx'), true);
});
