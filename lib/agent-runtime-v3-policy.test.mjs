...
test('single-pass automation finishes only after every dispatched task is accepted', () => {
  const state = {
    automated: true,
    singlePass: true,
    authorization: { objective: '生成并推送本次定时消息' },
  };
  assert.equal(automatedSinglePassCoverage(state, [{ id: 'task-1', status: 'RUNNING' }]), null);
  assert.equal(automatedSinglePassCoverage({ ...state, singlePass: false }, [{ id: 'task-1', status: 'COMPLETED' }]), null);
  assert.deepEqual(automatedSinglePassCoverage(state, [{
    id: 'task-1',
    status: 'COMPLETED',
    reviewSummary: '消息内容与格式已通过验收',
  }]), [{
    requirement: '生成并推送本次定时消息',
    taskIds: ['task-1'],
    evidence: '消息内容与格式已通过验收',
  }]);
});
...
test('task-level research declarations no longer control runtime research', () => {
  const task = {
  ...
  const denied = normalizeCoordinatorAction({ type: 'dispatch', tasks: [task] }, {
    members, remainingTasks: 8,
    authorization: { capabilities: ['workspace_read'], networkPolicy: 'forbidden' },
  });
  assert.equal(denied.tasks[0].webResearchRequired, false);

  const allowed = normalizeCoordinatorAction({ type: 'dispatch', tasks: [{
    ...task,
  ...
  assert.equal(allowed.tasks[0].webResearchRequired, false);
});

test('research is decided during execution rather than assigned by the coordinator', () => {
  const baseTask = {
    agentId: 'frontend', mode: 'executor', title: '制作主题网页',
    instruction: '使用核验资料制作单文件 HTML，最终页面不引入外部依赖。',
    acceptanceCriteria: 'index.html 可以离线预览。', reason: '前端负责页面交付。',
    expectedArtifacts: ['index.html'], skillId: 'responsive-page-builder',
  };
  const action = normalizeCoordinatorAction({
    type: 'dispatch', tasks: [{ ...baseTask, webResearchRequired: false }],
  }, {
    members, remainingTasks: 8,
    authorization: { capabilities: ['workspace_read', 'workspace_write', 'web_research'], networkPolicy: 'required' },
  });
  assert.equal(action.tasks[0].webResearchRequired, false);
});
...
test('dedicated image generation tasks must use executor mode', () => {
  assert.throws(() => normalizeCoordinatorAction({
    type: 'dispatch',
    tasks: [{
      agentId: 'frontend', mode: 'advisor', title: '生成首页配图',
      instruction: '生成一张首页配图并保存到 assets 目录。', acceptanceCriteria: '生成 PNG 图片。',
      reason: '前端负责页面视觉产物。', expectedArtifacts: ['assets/hero.png'],
    }],
  }, {
    members,
    remainingTasks: 8,
...
test('dispatch approval is the safe default while auto mode remains available', () => {
  ...
  assert.equal(dispatchRequiresApproval('REVIEW_DISPATCH', true), false);
});
