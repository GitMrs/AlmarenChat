...
test('required run research must be assigned before delivery work and self-contained output is not a denial', () => {
  const baseTask = {
  ...
  const action = normalizeCoordinatorAction({
    type: 'dispatch', tasks: [{ ...baseTask, webResearchRequired: true }],
  }, {
    members, remainingTasks: 8,
    authorization: { capabilities: ['workspace_read', 'workspace_write', 'web_research'], networkPolicy: 'required' },
  });
  assert.equal(action.tasks[0].webResearchRequired, true);
});
