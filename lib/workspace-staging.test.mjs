...
test('works can publish the same logical file name without overwriting each other', async () => {
  const projectRoot = await mkdtemp(path.join(os.tmpdir(), 'almaren-work-isolation-'));
  const base = { projectRoot, userId: 'user-1', spaceId: 'space-1', attempt: 1 };
  const first = { ...base, workId: 'work-1', taskId: 'task-1' };
  const second = { ...base, workId: 'work-2', taskId: 'task-2' };
  try {
    for (const [options, content] of [[first, 'first article'], [second, 'second article']]) {
      const formal = { projectRoot, userId: options.userId, spaceId: options.spaceId, workId: options.workId };
      const baseline = await snapshotWorkspace(formal);
      await prepareWorkspaceAttempt(options);
      await writeFile(workspaceAttemptFile(options, 'article.md').target, content, 'utf8');
      const entries = diffWorkspaceSnapshots(baseline, await snapshotWorkspace(options));
      await applyWorkspaceAttempt(options, baseline, entries);
    }

    const root = path.join(projectRoot, 'data', 'spaces', 'user-1', 'space-1', 'workspace', 'works');
    assert.equal(await readFile(path.join(root, 'work-1', 'article.md'), 'utf8'), 'first article');
    assert.equal(await readFile(path.join(root, 'work-2', 'article.md'), 'utf8'), 'second article');
  } finally {
    await rm(projectRoot, { recursive: true, force: true });
  }
});

test('works read and update one shared account strategy', async () => {
  const projectRoot = await mkdtemp(path.join(os.tmpdir(), 'almaren-shared-strategy-'));
  const base = { projectRoot, userId: 'user-1', spaceId: 'space-1', attempt: 1 };
  const first = { ...base, workId: 'work-1', taskId: 'task-1' };
  const second = { ...base, workId: 'work-2', taskId: 'task-2' };
  const sharedFile = path.join(projectRoot, 'data', 'spaces', 'user-1', 'space-1', 'workspace', 'shared', 'content-strategy.md');
  try {
    const firstBaseline = await snapshotWorkspace({ ...base, workId: first.workId });
    await prepareWorkspaceAttempt(first);
    await mkdir(path.dirname(workspaceAttemptFile(first, 'shared/content-strategy.md').target), { recursive: true });
    await writeFile(workspaceAttemptFile(first, 'shared/content-strategy.md').target, 'account v1', 'utf8');
    const firstEntries = diffWorkspaceSnapshots(firstBaseline, await snapshotWorkspace(first));
    await applyWorkspaceAttempt(first, firstBaseline, firstEntries);

    const secondBaseline = await snapshotWorkspace({ ...base, workId: second.workId });
    await prepareWorkspaceAttempt(second);
    assert.equal(await readFile(workspaceAttemptFile(second, 'shared/content-strategy.md').target, 'utf8'), 'account v1');
    await writeFile(workspaceAttemptFile(second, 'shared/content-strategy.md').target, 'account v2', 'utf8');
    const secondEntries = diffWorkspaceSnapshots(secondBaseline, await snapshotWorkspace(second));
    await applyWorkspaceAttempt(second, secondBaseline, secondEntries);

    assert.equal(await readFile(sharedFile, 'utf8'), 'account v2');
    await assert.rejects(
      readFile(path.join(projectRoot, 'data', 'spaces', 'user-1', 'space-1', 'workspace', 'works', 'work-2', 'shared', 'content-strategy.md'), 'utf8'),
      /ENOENT/
    );
  } finally {
    await rm(projectRoot, { recursive: true, force: true });
...
test('execution checkpoints are compressed, replaceable and private to one attempt', async () => {
  ...
  try {
    ...
    await discardWorkspaceAttempt(options);
    const skipped = await saveExecutionCheckpoint(options, { version: 1, totalIterations: 3 });
    assert.deepEqual(skipped, { bytes: 0, skipped: true });
  } finally {
    await rm(projectRoot, { recursive: true, force: true });
  }
});
