...
import { explicitlyForbidsResearchExecution } from './web-research-intent.mjs';
...
export function normalizeCoordinatorAction(content, options = {}) {
  ...
  const tasks = rawTasks.map((raw, index) => {
    ...
    if (webResearchRequired && explicitlyForbidsResearchExecution(taskText)) {
  ...
  const existingResearchTask = (options.existingTasks || []).some((task) => task.webResearchRequired === true || task.webResearchRequired === 1);
  if (options.authorization?.networkPolicy === 'required' && !existingResearchTask && !tasks.some((task) => task.webResearchRequired)) {
    throw new Error('当前目标明确要求联网，必须先安排联网研究任务');
  }
