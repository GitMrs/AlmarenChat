      ...
      properties: {
        ...
        tasks: {
          ...
          items: {
            type: 'object',
            additionalProperties: false,
            required: ['agentId', 'skillId', 'mode', 'title', 'instruction', 'acceptanceCriteria', 'reason'],
            properties: {
              agentId: { type: 'string' },
...
export function dispatchRequiresApproval(executionMode, automated = false) {
  return !automated && executionMode !== 'AUTO';
}
...
export function coordinatorTaskReviewInstructions(mode) {
  ...
  return '你是 AI 团队的 Coordinator。依据真实提交报告、文件差异和校验结果做 executor 执行任务的语义验收。图片产物按主文件名和用途匹配，模型返回的 png/jpg/jpeg/webp 扩展名差异不构成缺失；只有确实没有对应图片或比例、尺寸等验收标准不满足时才要求重新生成。' +
    format +
...
export function automatedSinglePassCoverage(state, tasks = []) {
  if (state?.automated !== true || state?.singlePass !== true || tasks.length === 0) return null;
  if (tasks.some((task) => task?.status !== 'COMPLETED')) return null;
  const taskIds = tasks.map((task) => task.id).filter(Boolean);
  const evidence = tasks
    .map((task) => text(task.reviewSummary || task.result, 500))
    .filter(Boolean)
    .join('；') || '本轮全部任务已通过逐项验收。';
  return authorizationRequirements(state.authorization).map((requirement) => ({
    requirement,
    taskIds,
    evidence,
  }));
...
export function normalizeCoordinatorAction(content, options = {}) {
  ...
  const tasks = rawTasks.map((raw, index) => {
    ...
    if ((skill.execution || skill.id === 'image-generator') && mode !== 'executor') {
    ...
    return {
      ...
      // Kept as a neutral database value for the current schema; execution no
      // longer uses a coordinator-provided research opt-in.
...
export function coordinatorAuthorization(proposal = {}) {
  ...
  const selectedSkill = proposal.skillSnapshot && typeof proposal.skillSnapshot === 'object'
    ...
    : null;
  const availableSkills = Array.isArray(proposal.availableSkills)
    ? proposal.availableSkills
      .filter((skill) => skill && typeof skill === 'object' && String(skill.id || '').trim())
      .map((skill) => JSON.parse(JSON.stringify(skill)))
      .slice(0, 32)
    : [];
  return {
    ...
    networkPolicy: ['forbidden', 'allowed', 'required', 'runtime'].includes(proposal.networkPolicy)
      ? proposal.networkPolicy
      : 'runtime',
    networkDecision: proposal.networkDecision === 'allowed' || proposal.networkPolicy === 'allowed'
      ? 'allowed'
      : proposal.networkPolicy === 'forbidden' ? 'denied' : 'pending',
    maxTasks: 8,
    ...(selectedSkill ? {
      selectedSkill,
