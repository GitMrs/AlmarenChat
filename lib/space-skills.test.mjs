...
test('ignores repository metadata without allowing other unsupported files', () => {
  const result = validateSpaceSkillPackage({
    ...
    files: [
      ...
      { path: '.DS_Store', content: 'finder metadata' },
      { path: 'LICENSE', content: 'MIT' },
      { path: 'SKILL.md', content: '---\nname: Novel Skill\n---\n执行小说工作流。' },
      { path: 'templates/.gitignore', content: '*' },
      { path: 'templates/progress.md', content: '# 进度' },
      { path: 'config/.env.example', content: 'API_KEY=replace-me' },
    ],
  });
  assert.deepEqual(result.manifest.files, ['SKILL.md', 'templates/progress.md', 'config/.env.example']);
...
test('accepts Skill ZIPs with more than 60 files when size limits are respected', () => {
  const archive = new AdmZip();
  archive.addFile('SKILL.md', Buffer.from('---\nname: Large Skill\n---\n执行工作流。'));
  for (let index = 0; index < 64; index += 1) {
    archive.addFile(`references/item-${index}.md`, Buffer.from(`条目 ${index}`));
  }
  const result = parseSpaceSkillArchive({ archive: archive.toBuffer(), sourceName: 'large-skill.zip' });
  assert.equal(result.manifest.files.length, 65);
});
