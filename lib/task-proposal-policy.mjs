...
const CODE_EXECUTION_PATTERN = /(?:运行|执行|调用).{0,16}(?:python|node|脚本|命令|测试|构建|编译)|(?:python|node|shell|bash|脚本|命令|测试|构建|编译)|(?:csv|逗号分隔).{0,40}(?:分析|统计|汇总|报告)|(?:分析|统计|汇总).{0,40}(?:csv|逗号分隔)|(?:run_skill|skill\s*脚本|脚本\s*入口|执行\s*skill)/i;
...
export function taskProposalWithServerCapabilities(proposal = {}, options = {}) {
  ...
  const normalizedDeclaredCapabilities = Array.isArray(proposal.capabilities)
    ? proposal.capabilities.map((capability) => capability === 'image_generation' ? 'image_generate' : capability)
    : null;
  const declaredCapabilities = normalizedDeclaredCapabilities
    ? [...new Set(normalizedDeclaredCapabilities.filter((capability) =>
    ...
    : null;
  const text = `${goal}\n${steps.join('\n')}\n${deliverables.join('\n')}`;
  const declaredNetworkPolicy = ['forbidden', 'allowed', 'required', 'runtime'].includes(proposal.networkPolicy)
  ...
  const networkPolicy = options.networkPolicyAuthoritative && declaredNetworkPolicy
    ...
    : explicitlyForbidsWebResearch(text)
      ? 'forbidden'
      : declaredNetworkPolicy === 'allowed' || declaredNetworkPolicy === 'required'
        ? declaredNetworkPolicy
        : 'runtime';
  ...
  if (networkPolicy !== 'forbidden'
    && !explicitlyForbidsWebResearch(text)) capabilities.push('web_research');
  return {
    ...proposal,
    goal,
    steps,
...
export function taskProposalWithTurnNetworkAuthorization(proposal = {}, allowWebSearch = false, userRequest = '') {
  // The user turn grants a bounded capability package. Whether research is
  // actually needed is decided later by the worker when a research tool is
  // requested, never by the proposal model.
  const secured = taskProposalWithServerCapabilities({
    ...proposal,
    networkPolicy: allowWebSearch ? 'allowed' : 'runtime',
    capabilities: [...new Set([...(Array.isArray(proposal.capabilities) ? proposal.capabilities : []), 'web_research'])],
  }, { networkPolicyAuthoritative: true });
  if (!explicitlyForbidsWebResearch(userRequest)) {
    secured.capabilities = [...new Set([...(secured.capabilities || []), 'web_research'])];
  } else {
    secured.networkPolicy = 'forbidden';
    secured.capabilities = (secured.capabilities || []).filter((capability) => capability !== 'web_research');
  }
  return secured;
}
