...
export const ADVISOR_MODEL_REQUEST_LIMIT = 8;
export const EXECUTOR_MODEL_REQUEST_LIMIT = 12;
...
export function defaultModelRequestLimit(tasks) {
  const taskBudget = tasks.reduce((total, task) => total + taskModelRequestLimit(task.mode), 0);
  return Math.min(48, Math.max(EXECUTOR_MODEL_REQUEST_LIMIT, taskBudget));
}

export function taskModelRequestLimit(mode) {
  return mode === 'advisor' ? ADVISOR_MODEL_REQUEST_LIMIT : EXECUTOR_MODEL_REQUEST_LIMIT;
}
