...
test('server rejects unknown declarations and restores explicit required capabilities', () => {
  ...
  assert.equal(proposal.networkPolicy, 'runtime');
});

test('Skill script requests require explicit code execution capability', () => {
  const proposal = taskProposalWithServerCapabilities({
    goal: '使用 creator-buddy Skill 脚本整理公众号趋势',
    steps: ['执行 Skill 脚本并生成报告'],
    deliverables: ['趋势报告'],
  });
  assert.equal(proposal.capabilities.includes('code_execute'), true);
});

test('legacy automation image capability is normalized when a run resumes', () => {
  const proposal = taskProposalWithServerCapabilities({
    goal: '生成公众号文章和封面',
    deliverables: ['article.md', 'cover.png'],
    capabilities: ['workspace_read', 'workspace_write', 'image_generation'],
    networkPolicy: 'forbidden',
  }, { networkPolicyAuthoritative: true });
  assert.deepEqual(proposal.capabilities, ['workspace_read', 'workspace_write', 'image_generate']);
});
...
test('the composer network switch is authoritative for new task proposals', () => {
  ...
  assert.equal(denied.networkPolicy, 'runtime');
  assert.deepEqual(denied.capabilities, ['workspace_read', 'workspace_write', 'web_research']);

  const allowed = taskProposalWithTurnNetworkAuthorization(modelProposal, true);
  assert.equal(allowed.networkPolicy, 'allowed');
  assert.deepEqual(allowed.capabilities, ['workspace_read', 'workspace_write', 'web_research']);
});

test('an explicit user research request overrides a model that only declares optional networking', () => {
  const proposal = taskProposalWithTurnNetworkAuthorization({
    goal: '制作单文件角色主题网页',
    steps: ['网页无需外部依赖即可打开'],
    deliverables: ['index.html'],
    capabilities: ['workspace_read', 'workspace_write'],
    networkPolicy: 'allowed',
  }, true, '先查《FX战士久留美》的相关资料，然后制作网页');
  assert.equal(proposal.networkPolicy, 'allowed');
  assert.deepEqual(proposal.capabilities, ['workspace_read', 'workspace_write', 'web_research']);
});

test('finding real character photos is treated as required external research', () => {
  const proposal = taskProposalWithTurnNetworkAuthorization({
    goal: '替换页面角色图片',
    steps: ['修改 index.html'],
    deliverables: ['index.html'],
    capabilities: ['workspace_read', 'workspace_write'],
    networkPolicy: 'allowed',
  }, true, '你找一下他们真实的角色照片换一下吧');
  assert.equal(proposal.networkPolicy, 'allowed');
  assert.equal(proposal.capabilities.includes('web_research'), true);
});
