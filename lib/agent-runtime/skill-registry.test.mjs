...
test('gzh-creator-suite is inferred for wechat article writing and enforces write capability', () => {
  const mediaAgent = { id: 'gl-zmtyy', name: '自媒体运营', description: '新媒体排版与运营' };
  const selected = resolveTaskSkill({
    agent: mediaAgent,
    text: '为公众号编写 article.md 深度长文并生成封面图',
    authorization: writeAuthorization,
  });
  assert.equal(selected.id, 'gzh-creator-suite');
  assert.match(selected.instructions, /article\.md/);
  assert.match(selected.instructions, /publish-info\.md/);
  assert.match(selected.instructions, /cover\.png/);
  assert.throws(() => resolveTaskSkill({
    requestedSkillId: 'gzh-creator-suite',
    agent: mediaAgent,
    text: '只分析不要写文件',
    authorization: writeAuthorization,
  }), /只能用于明确要求产出文章/);
...
test('script-developer is inferred for python and backend script development', () => {
  const backend = { id: 'professional-backend', name: '后端', description: '服务端与脚本开发' };
  const selected = resolveTaskSkill({
    agent: backend,
    text: '编写热榜数据拉取、历史排重与智能筛选脚本 shared/fetch_and_filter.py，输出 Markdown 格式数据',
    authorization: writeAuthorization,
  });
  assert.equal(selected.id, 'script-developer');
  assert.equal(validateSkillArtifacts(selected, [{ path: 'shared/fetch_and_filter.py', change: 'CREATED' }]).valid, true);
});
