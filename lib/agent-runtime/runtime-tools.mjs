...
import { isSharedWorkspacePath, storedWorkspaceRelativePath, workspaceFileWorkId } from '../space-work-paths.mjs';
...
async function workspaceContext({ projectRoot, userId, spaceId, workId, taskId, attempt }) {
  ...
  const [actualSpacesRoot, actualRoot, actualWorkspaceBase] = await Promise.all([
    realpath(spacesRoot),
    realpath(root),
    realpath(workspaceBase),
  ]);
  if (!actualRoot.startsWith(actualSpacesRoot + path.sep)) throw new Error('工作区目录超出空间范围');
  if (!actualWorkspaceBase.startsWith(actualSpacesRoot + path.sep)) throw new Error('工作区目录超出空间范围');
  return {
    root,
    actualRoot,
    workspaceBase,
...
export async function snapshotWorkspace(options) {
  ...
  if (context.workId && !context.isAttempt) {
    const sharedRoot = path.join(context.workspaceBase, 'shared');
    try {
      if ((await lstat(sharedRoot)).isDirectory()) await visit(sharedRoot, 'shared');
    } catch (error) {
      if (error?.code !== 'ENOENT') throw error;
    }
  }
...
async function resolveWorkspaceTarget(context, value, options) {
  ...
  const useSharedRoot = context.workId && !context.isAttempt && isSharedWorkspacePath(relativePath);
  const targetRoot = useSharedRoot ? context.workspaceBase : context.root;
  const actualTargetRoot = useSharedRoot ? context.actualWorkspaceBase : context.actualRoot;
  const target = relativePath ? path.resolve(targetRoot, relativePath) : targetRoot;
  if (target !== targetRoot && !target.startsWith(targetRoot + path.sep)) throw new Error('文件路径超出工作区');
  ...
  return { relativePath, target, actualTargetRoot };
}

async function readWorkspaceText(context, relativePath) {
  assertTextExtension(relativePath);
  const { target, actualTargetRoot } = await resolveWorkspaceTarget(context, relativePath);
  ...
  if (!actualTarget.startsWith(actualTargetRoot + path.sep)) throw new Error('文件路径超出工作区');
...
async function writeWorkspaceText(context, relativePath, content) {
  ...
  if (actualParent !== actualTargetRoot && !actualParent.startsWith(actualTargetRoot + path.sep)) {
...
async function listWorkspaceEntries(context, requestedPath) {
  const { relativePath, target, actualTargetRoot } = await resolveWorkspaceTarget(context, requestedPath, { allowRoot: true });
  const actualTarget = await realpath(target);
  if (actualTarget !== actualTargetRoot && !actualTarget.startsWith(actualTargetRoot + path.sep)) {
  ...
  if (!relativePath && context.workId && !context.isAttempt) {
    try {
      if ((await lstat(path.join(context.workspaceBase, 'shared'))).isDirectory()) {
        const sharedEntry = { path: 'shared', type: 'directory' };
        const existingIndex = result.findIndex((entry) => entry.path === 'shared');
        if (existingIndex >= 0) result[existingIndex] = sharedEntry;
        else result.push(sharedEntry);
      }
    } catch (error) {
      if (error?.code !== 'ENOENT') throw error;
    }
  }
  return result.sort((left, right) => left.path.localeCompare(right.path)).slice(0, MAX_LIST_ENTRIES);
}
...
export async function describeWorkspaceArtifact(options, value) {
  ...
  const actualTarget = await realpath(target);
  if (!actualTarget.startsWith(actualTargetRoot + path.sep)) throw new Error('产物路径超出工作区');
  const extension = path.extname(relativePath).toLowerCase();
  const shared = isSharedWorkspacePath(relativePath);
  return {
    id: randomUUID(),
    fileName: relativePath,
    mimeType: MIME_TYPES[extension] || 'text/plain; charset=utf-8',
    size: info.size,
    relativePath: storedWorkspaceRelativePath(options.workId, relativePath),
    workId: workspaceFileWorkId(options.workId, relativePath),
    shared,
