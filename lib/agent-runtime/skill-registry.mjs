...
export const BUILTIN_SKILLS = Object.freeze([
  ...
  Object.freeze({
    ...
    name: '脚本与代码开发',
    version: '1.0.0',
    description: '在工作区编写、修改与验证 Python、JavaScript 等自动化脚本与程序代码。',
    requiredCapabilities: ['workspace_read', 'workspace_write'],
    allowedTools: [...ALL_READ_TOOLS, 'write_file', 'patch_file', 'patch_files', 'run_check'],
    artifactExtensions: ['.py', '.js', '.mjs', '.cjs', '.ts', '.sh', '.json', '.txt', '.yaml', '.yml', '.env', '.md'],
    instructions: [
      '根据任务要求在工作区编写规范的代码或自动化脚本，使用正确的文件扩展名（如 .py, .mjs, .js 等）。',
      '确保代码具备完善的异常处理、超时控制与日志输出，支持独立直接执行。',
      '如果任务要求特定文件名（如 shared/fetch_and_filter.py），必须精确落盘到指定路径。',
    ].join('\n'),
    agentPattern: /(?:professional-backend|后端|coder|developer|engineer|程序员|代码|脚本)/i,
  }),
]);
...
const CODE_SCRIPT_PATTERN = /(?:\.py\b|\.js\b|\.ts\b|\.mjs\b|\.cjs\b|\.sh\b|python|node\.?js|脚本|编写代码|编写脚本|写脚本|自动化脚本|爬虫|爬取|拉取脚本|接口请求脚本)/i;
...
function inferredSkillId(text, agent, capabilities) {
  ...
  if (canWrite && writeIntent && CODE_SCRIPT_PATTERN.test(text)) {
    const scriptSkill = builtinSkill('script-developer');
    if (skillSupportsAgent(scriptSkill, agent)) return scriptSkill.id;
...
export function resolveTaskSkill({ requestedSkillId, agent, text, authorization, additionalSkills = [] } = {}) {
  ...
  if (skill.id === 'script-developer' && !needsWorkspaceWrite(text)) {
    throw new Error('脚本与代码开发 Skill 只能用于明确要求产出代码或脚本的任务');
  }
  if (skill.id === 'document-writer' && (!DOCUMENT_PATTERN.test(text) || !needsWorkspaceWrite(text) || (CODE_SCRIPT_PATTERN.test(text) && !/\.md\b/i.test(text)))) {
