...
export const BUILTIN_SKILLS = Object.freeze([
  Object.freeze({
    ...
    allowedTools: [...ALL_READ_TOOLS, 'generate_image'],
    artifactExtensions: ['.png', '.jpg', '.jpeg', '.webp'],
    instructions: [
      '只生成当前步骤明确需要的图片，最多调用 generate_image 两次。',
      '图片必须保存到 assets 目录，并使用简短、可读的英文文件名。',
      '生成后立即提交文件路径、用途和实际尺寸，不得在回复中输出 Base64。',
    ].join('\n'),
    agentPattern: /(?:professional-frontend|前端|ui|视觉|设计|design)/i,
  }),
  Object.freeze({
  ...
  Object.freeze({
    ...
    requiredCapabilities: ['workspace_read', 'workspace_write'],
    allowedTools: [...ALL_READ_TOOLS, 'write_file', 'patch_file', 'patch_files', 'run_check', 'generate_image'],
