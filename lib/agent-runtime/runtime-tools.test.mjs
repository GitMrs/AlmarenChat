...
test('read_file reads normal workspace files once and paginates only oversized files', async () => {
  ...
  const options = { projectRoot, userId: 'user-1', spaceId: 'space-1' };
  try {
    await executeWorkspaceTool(options, 'write_file', { path: 'normal.md', content: 'a'.repeat(56_830) });
    const normal = await executeWorkspaceTool(options, 'read_file', { path: 'normal.md' });
    assert.equal(normal.content.length, 56_830);
    ...
    assert.equal(first.content.length, 64_000);
    assert.equal(first.hasMore, true);
    assert.equal(second.content.length, 86_000);
