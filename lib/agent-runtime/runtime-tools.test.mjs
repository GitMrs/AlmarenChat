...
test('work workspace tools expose and read the space-level shared directory', async () => {
  const projectRoot = await mkdtemp(path.join(os.tmpdir(), 'almaren-runtime-'));
  const baseOptions = { projectRoot, userId: 'user-1', spaceId: 'space-1' };
  const workOptions = { ...baseOptions, workId: 'work-1' };
  try {
    await executeWorkspaceTool(baseOptions, 'write_file', {
      path: 'shared/content-strategy.md',
      content: '# Shared strategy',
    });
    await executeWorkspaceTool(workOptions, 'write_file', {
      path: 'article.md',
      content: '# Work article',
    });

    const root = await executeWorkspaceTool(workOptions, 'list_files', { path: '' });
    const shared = await executeWorkspaceTool(workOptions, 'list_files', { path: 'shared' });
    const strategy = await executeWorkspaceTool(workOptions, 'read_file', {
      path: 'shared/content-strategy.md',
    });

    assert.deepEqual(root.entries.map((entry) => entry.path), ['article.md', 'shared']);
    assert.deepEqual(shared.entries, [{ path: 'shared/content-strategy.md', type: 'file' }]);
    assert.equal(strategy.content, '# Shared strategy');
  } finally {
    await rm(projectRoot, { recursive: true, force: true });
  }
});
