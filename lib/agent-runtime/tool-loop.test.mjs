...
test('successful mutation arguments are compacted before the next model request', async () => {
  ...
  await runToolLoop({
    ...
    requestCompletion: async (messages) => {
      ...
      assert.match(calls.patch_file.search, /6 个字符已省略/);
      assert.match(calls.patch_file.replacement, /5 个字符已省略/);
      assert.match(calls.patch_files.edits[0].search, /3 个字符已省略/);
      assert.match(calls.patch_files.edits[0].replacement, /3 个字符已省略/);
      return { content: '完成' };
    },
    executeTool: async (name, args) => {
