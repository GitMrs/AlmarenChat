...
import {
  explicitlyForbidsResearchExecution,
  explicitlyForbidsWebResearch,
  needsWebResearch,
} from './web-research-intent.mjs';
...
test('negative research requirements stay offline', () => {
  ...
  assert.equal(needsWebResearch('文档为纯 Markdown 产物，无联网内容'), false);
  assert.equal(needsWebResearch('离线完成，不引入外部链接内容'), false);
  assert.equal(needsWebResearch('快捷入口为新建任务、日历、消息、搜索 4 个，全程不需要联网'), false);
...
test('explicit offline requirements are a hard denial unless later requirements need research', () => {
  assert.equal(explicitlyForbidsWebResearch('纯文本 Markdown，不联网、不引入外部链接'), true);
  assert.equal(explicitlyForbidsWebResearch('页面包含搜索入口，全程不需要联网'), true);
  assert.equal(explicitlyForbidsWebResearch('无需联网运行，但仍需搜索最新资料'), false);
});

test('research execution denial does not confuse a self-contained artifact with offline research', () => {
  assert.equal(explicitlyForbidsResearchExecution('不要联网或搜索，只使用现有资料'), true);
  assert.equal(explicitlyForbidsResearchExecution('单文件 HTML，无跨域依赖，不引入外部资源'), false);
  assert.equal(explicitlyForbidsResearchExecution('先搜索资料，最终页面无需联网即可打开'), false);
});
...
test('conditional source requirements do not trigger external research', () => {
  assert.equal(needsWebResearch('若引用外部事实请附来源清单与核验说明'), false);
  assert.equal(needsWebResearch('如果使用外部资料，再补充来源链接'), false);
});
