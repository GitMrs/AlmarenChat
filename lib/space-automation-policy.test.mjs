...
test('automation completion actions are restricted to matching scenario templates', () => {
  ...
  assert.deepEqual(normalizeAutomationCompletion({
    completionAction: 'WEBHOOK_NOTIFY', completionConfig: { target: 'PERSONAL_QQ' },
  }), {
    completionAction: 'WEBHOOK_NOTIFY', completionConfig: { target: 'PERSONAL_QQ' },
  });
  assert.deepEqual(normalizeAutomationCompletion({
    completionAction: 'WEBHOOK_NOTIFY', completionConfig: { target: 'CUSTOM_WEBHOOK', url: 'https://example.com/hook', bodyTemplate: '{"content":"{{content.text}}"}' },
  }), {
    completionAction: 'WEBHOOK_NOTIFY', completionConfig: { target: 'CUSTOM_WEBHOOK', url: 'https://example.com/hook', bodyTemplate: { content: '{{content.text}}' } },
  });
  assert.throws(() => normalizeAutomationCompletion({
    completionAction: 'WEBHOOK_NOTIFY', completionConfig: { target: 'CUSTOM_WEBHOOK', url: 'http://example.com/hook', bodyTemplate: '{}' },
  }), /HTTPS/);
  assert.throws(() => normalizeAutomationCompletion({
    completionAction: 'WEBHOOK_NOTIFY', completionConfig: { target: 'CUSTOM_WEBHOOK', url: 'https://127.0.0.1/hook', bodyTemplate: '{}' },
  }), /本机或内网/);
  assert.throws(() => normalizeAutomationCompletion({
    completionAction: 'WEBHOOK_NOTIFY', completionConfig: { target: 'CUSTOM_WEBHOOK', url: 'https://example.com/hook', bodyTemplate: '{"content":}' },
  }), /JSON 模板格式无效/);
});
