...
export const AUTOMATION_COMPLETION_ACTIONS = Object.freeze(['NONE', 'WECHAT_CREATE_DRAFT', 'WEBHOOK_NOTIFY']);
const WECHAT_THEME_IDS = new Set(['fresh-green', 'editorial-red', 'midnight-gold']);
const WEBHOOK_TARGETS = new Set(['PERSONAL_QQ', 'CUSTOM_WEBHOOK']);

export function isUnsafeWebhookHostname(hostname) {
...
export function normalizeWebhookConfig(inputConfig = {}, fallbackConfig = {}) {
  const target = String(inputConfig.target ?? fallbackConfig.target ?? 'PERSONAL_QQ').toUpperCase();
  if (!WEBHOOK_TARGETS.has(target)) throw new Error('Webhook 通知目标无效');
  if (target === 'PERSONAL_QQ') return { target };
  const webhookId = String(inputConfig.webhookId ?? fallbackConfig.webhookId ?? '').trim();
  if (webhookId) return { target, webhookId };
  const url = String(inputConfig.url ?? fallbackConfig.url ?? '').trim();
  let parsed;
  try { parsed = new URL(url); } catch { throw new Error('自定义 Webhook 必须是有效的 HTTPS 地址'); }
  if (parsed.protocol !== 'https:') throw new Error('自定义 Webhook 只支持 HTTPS 地址');
  if (parsed.username || parsed.password) throw new Error('自定义 Webhook 地址不能包含账号或密码');
  if (isUnsafeWebhookHostname(parsed.hostname)) throw new Error('自定义 Webhook 不能指向本机或内网地址');
  const rawTemplate = inputConfig.bodyTemplate ?? fallbackConfig.bodyTemplate;
  let bodyTemplate = rawTemplate;
  if (typeof rawTemplate === 'string') {
    if (rawTemplate.length > 12_000) throw new Error('Webhook JSON 模板不能超过 12000 字');
    try { bodyTemplate = JSON.parse(rawTemplate); } catch { throw new Error('Webhook JSON 模板格式无效'); }
  }
  if (bodyTemplate === null || typeof bodyTemplate !== 'object') {
    throw new Error('Webhook JSON 模板必须是对象或数组');
  }
  return { target, url: parsed.toString(), bodyTemplate };
}
...
export function normalizeAutomationCompletion(input = {}, templateId = null, fallback = {}) {
  ...
  if (completionAction === 'WEBHOOK_NOTIFY') {
    ...
    return { completionAction, completionConfig: normalizeWebhookConfig(inputConfig, fallbackConfig) };
  }
  const inputConfig = input.completionConfig && typeof input.completionConfig === 'object' ? input.completionConfig : {};
  const fallbackConfig = fallback.completionConfig && typeof fallback.completionConfig === 'object' ? fallback.completionConfig : {};
...
export const AUTOMATION_EXECUTION_MODES = Object.freeze(['PROMPT', 'SCRIPT_ANALYSIS', 'SCRIPT_DIRECT']);
export const SUPPORTED_AUTOMATION_SCRIPT_EXTENSIONS = new Set(['.js', '.mjs', '.cjs', '.py']);

export function normalizeAutomationExecutionMode(value) {
  const mode = String(value || 'PROMPT').toUpperCase();
  if (!AUTOMATION_EXECUTION_MODES.includes(mode)) {
    throw new Error('自动化执行模式无效');
  }
  return mode;
}

export function normalizeAutomationScriptPath(value, executionMode = 'PROMPT') {
  if (executionMode === 'PROMPT') return null;
  const raw = String(value || '').trim().replaceAll('\\', '/');
  if (!raw) {
    throw new Error('脚本执行模式必须指定脚本路径');
  }
  if (raw.startsWith('/') || raw.startsWith('~') || /^[a-z][a-z0-9+.-]*:/i.test(raw)) {
    throw new Error('脚本只允许空间工作区相对路径');
  }
  const parts = raw.split('/');
  if (parts.some((part) => !part || part === '.' || part === '..')) {
    throw new Error('脚本路径不安全');
  }
  const ext = (raw.includes('.') ? raw.slice(raw.lastIndexOf('.')) : '').toLowerCase();
  if (!SUPPORTED_AUTOMATION_SCRIPT_EXTENSIONS.has(ext)) {
    throw new Error(`不支持的脚本类型：${ext || '无扩展名'}，仅支持 .mjs, .js, .cjs, .py`);
  }
  return parts.join('/');
}
