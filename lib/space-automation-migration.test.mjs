...
test('space automation migrations create an auditable calendar schedule', async () => {
  ...
  try {
    ...
    for (const migrationName of [
      ...
      '20260921180000_add_space_automation_execution_mode',
      '20260922140000_add_automation_delivery_receipts',
    ]) {
    ...
    const automationExecutionColumns = new Set(db.prepare('PRAGMA table_info("SpaceAutomationExecution")').all().map((column) => column.name));
    for (const column of ['workId', 'result', 'resultHash', 'deliveryStatus', 'deliveryAttempts', 'deliveryError', 'deliveryNextAttemptAt', 'deliveredAt']) {
      assert.equal(automationExecutionColumns.has(column), true, `missing ${column}`);
    }
    assert.equal(indexes.has('SpaceAutomationExecution_deliveryStatus_deliveryNextAttemptAt_idx'), true);
