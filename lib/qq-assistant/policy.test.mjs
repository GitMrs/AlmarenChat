...
test('QQ proactive quota errors are shown as a readable warning', () => {
  assert.deepEqual(qqAssistantErrorNotice(
    'API Error [/v2/users/openid/messages]: 消息发送失败，召回消息已达区间上限 code=40034122'
  ), {
    tone: 'warning',
    message: 'QQ 主动消息额度暂时受限，当前连接正常；额度恢复后可再次推送。',
  });
  assert.equal(qqAssistantErrorNotice(null), null);
  assert.equal(qqAssistantErrorNotice('x'.repeat(500)).message.length, 300);
});
