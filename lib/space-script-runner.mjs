...
import { createLinuxBubblewrapInvocation } from './linux-bubblewrap.mjs';
...
function scriptInvocation({ actualWorkspace, actualScript, interpreter, env, network }) {
  if (process.platform !== 'linux') {
    return {
      backend: 'host',
      executable: interpreter.executable,
      args: [...interpreter.args, actualScript],
      cwd: actualWorkspace,
    };
  }

  const scriptRelative = path.relative(actualWorkspace, actualScript).split(path.sep).join('/');
  return createLinuxBubblewrapInvocation({
    workspaceRoot: actualWorkspace,
    scriptRoot: actualWorkspace,
    scriptRelative,
    executable: interpreter.executable,
    executableArgs: interpreter.args,
...
function processEnvironment(workspaceRoot, extraEnv) {
  if (process.platform === 'linux') {
    return {
 ...
 * @param {Record<string, string>} [options.env] - 传给脚本的额外环境变量
 * @param {boolean} [options.network] - Linux 沙箱是否保留网络访问
 */
export async function executeSpaceScript({
  ...
  const invocation = scriptInvocation({ actualWorkspace, actualScript, interpreter, env, network });
  ...
  return new Promise((resolve) => {
    const child = spawn(invocation.executable, invocation.args, {
      ...
      env: processEnvironment(actualWorkspace, env),
    });
