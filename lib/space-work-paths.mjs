...
export function isSharedWorkspacePath(logicalRelativePath) {
  const logical = String(logicalRelativePath || '').replaceAll('\\', '/').replace(/^workspace\//, '');
  return logical === 'shared' || logical.startsWith('shared/');
}

export function storedWorkspaceRelativePath(workId, logicalRelativePath) {
  const logical = String(logicalRelativePath || '').replaceAll('\\', '/').replace(/^workspace\//, '');
  return workId && !isSharedWorkspacePath(logical)
    ? path.posix.join('workspace', 'works', safeSpaceWorkId(workId), logical)
    : path.posix.join('workspace', logical);
}

export function workspaceFileWorkId(workId, logicalRelativePath) {
  return workId && !isSharedWorkspacePath(logicalRelativePath) ? safeSpaceWorkId(workId) : null;
}
