...
test('agent memory schema and API keep memory scoped by user and agent', async () => {
  ...
  assert.match(schema, /agentType\s+String\s+@default\("BASIC"\)/);
...
test('confirmed agent memory reaches every employee execution entry point', async () => {
  ...
  assert.match(worker, /agent\.agentType === 'EMPLOYEE'/);
  assert.match(worker, /task\.title.*task\.instruction.*task\.acceptanceCriteria/s);
...
test('agent growth records are managed from the editor instead of the public detail page', async () => {
  ...
  assert.match(editor, /creationMode === 'pro'/);
...
test('basic agents are gated from employee memory reads and writes', async () => {
  const [loader, service, memoryRoute, agentRoute] = await Promise.all([
    readFile(path.join(root, 'lib/agent-memory.ts'), 'utf8'),
    readFile(path.join(root, 'lib/agent-memory-service.ts'), 'utf8'),
    readFile(path.join(root, 'app/api/agents/[agentId]/memory/route.ts'), 'utf8'),
    readFile(path.join(root, 'app/api/agents/[agentId]/route.ts'), 'utf8'),
  ]);
  assert.match(loader, /agentType: 'EMPLOYEE'/);
  assert.match(service, /agentType: 'EMPLOYEE'/);
  assert.match(memoryRoute, /普通 Agent 不支持员工记忆/);
  assert.match(agentRoute, /agentType: _agentType/);
});
